...
*** =instructions
- Type `library(mosaic)` to load the *mosaic* package into your workspace.
- Use the `favstats()` function to examine the distribution of post weights by treatment in the `anorexia` data set.
...
To test if average post-treatment weight differs among the treatment categories, we can use the `aov()` function.
...
*** =instructions
- Use the format in the lesson to test if there is a difference in average post weights among the three treatment categories, name these results `anova.results`.
...
The horizontal lines show 95% confidence intervals for each difference in means. If the confidence interval contains zero, it is plausable that the population means are *equal* for that that pair. If the interval does not contain zero, the two means are significantly different.
