...
You may have noticed from the previous exercise that the `sex` variable is coded as numeric, but this is incorrect.

`sex` only takes on two values:

* 1 = respondent is female
* 2 = respondent is male 

What is the correct variable type of `sex`?
...
*** =sct
```{r}
msg_bad <- "That is not correct. Since 'sex' only takes on two values it should be coded as a categorical variable (factor)."
msg_success <- "Exactly!"
...
R thinks that the `sex` variable is 'numeric' but really it should be coded as 'factor' because it is a **categorical variable**. 

In this exercise we are going to recode the `sex` variable from a numeric to a factor variable type.


*** =instructions
- The first line of code in your script will recode `sex` from a 'numeric' variable to its' correct variable type, 'factor'.
- Use the `str()` function on `CPS85`.
- Click the 'Submit Answer' Button and take a look at the R output in the console. Make sure the `sex` variable was correctly recoded to a factor variable.
*** =hint
...
- Find summary statistics of the `age` variable in `CPS85` using the `favstats()` function.

*** =hint
Follow the format in the lesson substituting `CPS85` for dataset and the specified variable for quant_var.
...
# Use the favstats() function to explore age in the CPS85 data set 
...
*Note: the quantitative variable (`wage`) goes on the left hand side of the tilde, and the categorical grouping variable (`sex`) goes on the right hand side.*
*** =instructions
- Type `library(mosaic)` to load the *mosaic* package into your workspace.
- Use the `favstats()` function to compare the subjects' `wage` (from the `CPS85` data set) by their `sex`.
...
*** =instructions
- Create a contingency table with variables `married` and `sex` from the `CPS85` data set.
*** =hint
...
*** =sct
```{r}
test_student_typed("barplot(table(CPS85$married))", not_typed_msg = "Make sure you follow the format in the lesson with 'CPS85' instead of 'dataset' and 'married' instead of 'categ_var'.")
```
...
For this exercise, we are going to visualize the relationship between `educ` and `wage` in the `CPS85` data set:
...
*** =instructions
- Use the `plot()` function to create a scatterplot of `educ` and `wage`.
...
**This is the last exercise for Lab 3**
...
Create side-by-side boxplots to compare the `wage` of respondents in the `CPS85` data set by whether they are male of female `sex`.

*** =hint
Use format in the lesson with `wage` = `quant_var`, `CPS85` = `dataset`, `sex` = `categ_var`
*** =pre_exercise_code
```{r}
library(mosaicData)
CPS85$sex <- factor(CPS85$sex, labels = c("female", "male"))
...
*** =sample_code
```{r}
# Create side-by-side boxplots with quant_var = wage and categ_var = sex
...
**This is the last exercise of Lab 3**
...
msg_success <- "Good answer! You finished Lab 3 practice course."
