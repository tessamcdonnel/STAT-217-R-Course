...
test_function("library", args = "package", incorrect_msg = "Type `library(mosaic)` below the first comment to load the mosaic package.")
...
Suppose that in *my* sample, the mean chick weight was 125.65 grams.

How does this compare to the **population mean**?

The `favstats()` results for the population (entire data set) are stored in an object called `pop_results`; in your R console type `pop_results` to view.
